...
from gpytorch.kernels import ScaleKernel, RBFKernel, PolynomialKernelGrad, Kernel
...
def one_experiment(
    ...
    if data_path is None:
        data_path = (
            os.getcwd()
            + "/polyatomic_complexes/dataset/photoswitches/photoswitches.csv"
        )
    if fig_path is None:
        fig_path = (
            os.getcwd()
            + f"/polyatomic_complexes/results/Photoswitches/{encoding}_Results/confidence_mae_model_{encoding}_{target}.png"
        )
