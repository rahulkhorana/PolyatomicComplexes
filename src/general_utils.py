...
class GeneralComplexUtils:
    ...
    def topological_disjoint_union(self, gluing_map:defaultdict, skeleton:np.ndarray, particle:np.ndarray):
    ...
    def get_nsphere_and_limits(self, radius:np.float32, center:np.ndarray) -> Tuple[list, list]:
    ...
    def get_nsphere_and_sampled_boundary(self, radius:np.float32, center:np.ndarray, sample:int, eps:np.float32) -> [list, np.ndarray]:
        representation = NSphere(r=radius, center=center)
        nsphere = representation.approximate()
        dim = center.shape[0]
        boundary = set()
        ...
        while len(boundary) < dim:
            if numit >= self.cutoff:
                ...
                if remain == dim:
                    ...
                    for _ in range(rem):
                        ...
                        if type(boundary[-1]) is float or type(boundary[-1]) is np.float64 or type(boundary[-1]) is np.float32:
            ...
            for i in range(sample):
                ...
                if np.abs(norm - radius) < eps and len(boundary) < dim and type(boundary) is set:
                    rand_vector = tuple(rand_vector)
                    boundary.add(rand_vector)
                elif len(boundary) >= dim:
                    break
                elif type(boundary) is list and np.abs(norm - radius) < eps and len(boundary) < dim:
        ...
        assert len(boundary) == dim and len(boundary[0]) == dim and type(boundary) is np.ndarray and boundary.shape == (dim, dim)
